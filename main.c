...
// fun��o para verificar se uma matr�cula est� presente no arquivo "matriculas.txt" e retornar o nome correspondente
char *obterNomePorMatricula(const char *matricula) {
    ...
    if (arquivoMatriculas == NULL || arquivoNomes == NULL) {
        printf("Erro ao abrir o arquivo de matr�culas ou nomes.\n");
        exit(1);
    }
    ...
    while (fgets(matriculaArquivo, sizeof(matriculaArquivo), arquivoMatriculas) != NULL) {
        ...
        if (strcmp(matricula, matriculaArquivo) == 0) {
            ...
            fclose(arquivoNomes);
            return strdup(nome); // retorna uma c�pia alocada dinamicamente do nome encontrado
        }

        // se a matr�cula n�o corresponder, ler e descartar o pr�ximo nome
        fgets(nome, sizeof(nome), arquivoNomes);
    }

    fclose(arquivoMatriculas);
    fclose(arquivoNomes);
    return NULL; // matr�cula n�o encontrada
}
...
while (fscanf(fileSD, "%s %s %d %f %f %f %f %f %f %f %f %f\n", alunoSD[numAlunosSD].matricula, alunoSD[numAlunosSD].nome, &alunoSD[numAlunosSD].faltas, &alunoSD[numAlunosSD].prova1, &alunoSD[numAlunosSD].prova2, &alunoSD[numAlunosSD].projeto1,&alunoSD[numAlunosSD].projeto2,&alunoSD[numAlunosSD].projeto3,&alunoSD[numAlunosSD].projeto4,&alunoSD[numAlunosSD].projeto5,&alunoSD[numAlunosSD].NPresenca, &alunoSD[numAlunosSD].notaFinal) == 12) {
    ...
    printf("Caso seja aluno, insira sua matr�cula:\n ");
    printf("Caso seja professor apenas aperte enter\n");
    //comando que recebe matricula para compara��o
    fgets(matricula, sizeof(matricula), stdin);
    matricula[strcspn(matricula, "\n")] = '\0'; // remover o caractere de nova linha '\n' do final

    // verificar se a matr�cula est� presente no arquivo de matr�culas
   char *nomeAluno = obterNomePorMatricula(matricula);
        if (nomeAluno != NULL) {
    printf("Login de aluno realizado com sucesso!\n");
    printf("\nAluno logado: %s\n\n", nomeAluno);
    free(nomeAluno); // liberar a mem�ria alocada dinamicamente(nome encontrado)

    printf("Escolha a disciplina para visualisar suas notas:\n");
    printf("1. Algoritmos e Programa��o II\n");
    printf("2. C�lculo II\n");
    printf("3. Geometria Anal�tica\n");
    printf("4. Carreira e Viv�ncia na Computa��o\n");
    printf("5. Matem�tica Discreta\n");
    printf("6. Sistemas Digitais\n");
    printf("Op��o: ");
    scanf("%d", &opcao);
//case para op��o alunos
    switch (opcao) {
        //case ap2
        case 1:
            printf("Disciplina selecionada: Algoritmos e Programa��o II\n");
            mostrarDadosAluno(aluno, numAlunos, matricula);
            break;
        case 2:
            //case cal
            printf("Disciplina selecionada: C�lculo II\n");
            mostrarDadosAlunoCal(alunoCal, numAlunosCal, matricula);
            break;
        case 3:
            //case ga
            printf("Disciplina selecionada: Geometria Anal�tica\n");
            mostrarDadosAlunoGa(alunoGa, numAlunosGa, matricula);
            break;
        case 4:
            //case cvc
            printf("Disciplina selecionada: Carreira e Viv�ncia na Computa��o\n");
            mostrarDadosAlunoCVC(alunoCVC, numAlunosCVC, matricula);
            break;
        case 5:
            //case md
            printf("Disciplina selecionada: Matem�tica Discreta\n");
        ...
        //caso seja professor
        printf("Voc� � um professor.\n");
        printf("Por favor, escolha sua op��o:\n");
        printf("1. Ana Paula\n");
        printf("2. Fernando\n");
        printf("3. Joslaine\n");
        printf("4. Marcelo\n");
        ...
        // login dos professores com senha e op��o de disciplina
        switch (opcao) {
            case 1:
                //case Ana Paula
    ...
    funcaoAnaPaula(aluno, numAlunos);
            ...
            case 3:
                ...
                if (strcmp(senha, "3456") == 0) {
                    ...
                    printf("Escolha a disciplina:\n");
                    printf("1. Carreira e Viv�ncia na Computa��o\n");
                    printf("2. Matem�tica Discreta\n");
                    printf("Op��o: ");
                    scanf("%d", &opcao);
                  if (opcao == 1) {
